...
if __name__ == '__main__':
    with open(SCRIPT_DIRECTORY + 'databaseBackupLog.txt', 'w') as log:
        ...
        try:
            ...
            filename = FILENAME_PREFIX + timestamp + '.sql'  # create a filename based on our prefix and timestamp
            filenamePath = SCRIPT_DIRECTORY + filename
            dumpCMD = f'mysqldump -h {DB_HOST} -u {DB_USER} -p{DB_PW} {DB_NAME} > "{filenamePath}"'  # create the SQL dump command to output to our given filename. password needs to immediately follow the -p
            print(f'DBUG: Creating SQL dump with the following command: {dumpCMD}')
            print(f'DBUG: Creating SQL dump with the following command: {dumpCMD}', file=log)
            os.system(dumpCMD)  # execute the dump command on the system
        except Exception as er:
            print(f'ERROR while creating SQL dump: {er}')
            print(f'ERROR while creating SQL dump: {er}', file=log)

        print(f'INFO: Successfully created SQL file, attempting to upload to Google drive folder named "{GOOGLE_DRIVE_FOLDER_NAME}"')
        print(f'INFO: Successfully created SQL file, attempting to upload to Google drive folder named "{GOOGLE_DRIVE_FOLDER_NAME}"',file=log)
        nextPageFolderToken = ''
        while nextPageFolderToken is not None:
            ...
            if folder:
                ...
                try:
                    ...
                    print(f'INFO: SQL backup file "{filename}" was uploaded to Google Drive folder with ID {folderID}, resulting file ID is {uploadFile.get("id")}')
                    print(f'INFO: SQL backup file "{filename}" was uploaded to Google Drive folder with ID {folderID}, resulting file ID is {uploadFile.get("id")}', file=log)
                ...
                try:
                    ...
                    print('DBUG: Local SQL file was successfully deleted after upload')
                    print('DBUG: Local SQL file was successfully deleted after upload', file=log)
                except Exception as er:
                    print(f'ERROR while deleting local SQL file after upload: {er}')
                    print(f'ERROR while deleting local SQL file after upload: {er}', file=log)
